<div class="card-body">
    ...
    <table class="table table-hover table-striped mb-3">
        ...
        @forelse($categories as $category)
            <tr>
                <td>{{ $category->name }}</td>
                <td class="text-center" style="color:{{ $category->color }} "><i class="fas fa-circle"></i></td>
                <td class="text-center">
                    <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
                        <a href="{{ route('user.show',$category->user->id) }}" data-toggle="popover" title="{{ $category->user->name }}" data-content="<i class='fas fa-calendar-alt'></i> Em: {{date('d/m/Y',strtotime($category->updated_at))}} às {{date('H:i',strtotime($category->updated_at))}}h" class="btn btn-dark btn-xs"><i class="fas fa-user"></i></a>
                        <div class="btn-group" role="group">
                            ...
                            <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                <a class="dropdown-item" href="{{ route('category.edit',$category->id) }}"><i class="fas fa-edit"></i> Editar</a>
                                <div class="dropdown-divider"></div>
                                {!! Form::model($category, ['method' => 'delete', 'route' => ['category.destroy', $category->id], 'class' =>'form-delete']) !!}
